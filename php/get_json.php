<?php
	...
	if(isset($_GET['password_of_base']) && isset($_GET["folder_of_base"]) && isset($_GET["name_of_base"]))
	{
		
		
		$folder_name = $_GET['folder_of_base'];
		if(folder_exist("../data/".$folder_name)===false)
		{
			die("get_json.php::error::folder [$folder_name] not found");
			//mkdir("../images/".$folder_name, 0777);
		}
		
		$pswd = $_GET['password_of_base'];
		
		if($pswd!= getCurrentRightPassword($folder_name))
		{
			die("get_json.php::error::Wrong password [$pswd] for folder [$folder_name]");
		}			
		
		
		$filename = $_GET['name_of_base'];
		$fulljsonname ="../data/".$folder_name."/$filename.json";
		if(file_exists($fulljsonname)===false)
		{
			die("get_json.php::error::file [$$fulljsonname] not found");
		...
		{
			die("get_json.php.php::error::file [$$fullpngname] not found");
			
		}
		
		echo file_get_contents($fulljsonname);
...
**/			
	...
	{ 
		echo "get_json.php::error::wrong parameters error";
